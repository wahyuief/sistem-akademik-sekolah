...
class Siswa extends CI_Controller {
	...
	public function get_data()
	{
		if ($this->input->get('wahyugantengbanget') === 'c0meok210ei1ej21i') {
			...
			$users = $this->aauth->list_users(6, $rowperpage, $start, true, "$columnName $columnSortOrder", $searchQuery);
			...
			foreach ($users as $row) {
				$user[] = array(
					...
					'opsi' => '<a href="'.base_url('admin/siswa/do_update/'.$row->id).'" class="linkEdit" fullname="'.$row->fullname.'" username="'.$row->username.'" email="'.$row->email.'" status="'.$row->banned.'">Edit</a>'
				);
			}
			$data = array(
	...
	public function do_insert()
	{
		...
		$this->form_validation->set_rules('username', 'NISN', 'trim|required|min_length[4]|is_unique[aauth_users.username]|numeric');
		...
		if ($this->form_validation->run() === FALSE) {
			...
			if ($user = $this->aauth->create_user($email, $password, $username, $fullname, 6, true)) {
	...
	public function do_update($user_id)
	{
		...
		$this->form_validation->set_rules('username', 'NISN', 'trim|required|min_length[4]|numeric');
