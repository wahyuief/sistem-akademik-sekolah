...
class Aauth {
	 ...
	 */
	public function create_user($email, $pass, $username = false, $fullname, $group, $sekolah = false) {
	 ...
	 * @return bool Update fails/succeeds
	 */
	public function update_user($user_id, $email = false, $pass = false, $username = false, $fullname = false, $kelas = false, $subkelas = false) {
	 ...
	 */
	public function list_users($group_par = false, $limit = false, $offset = false, $include_banneds = false, $sort = false, $search = false, $alumni = 0, $custom_where = false) {
		...
		if ($search) {
			$this->aauth_db->where($search);
		}
		
		$this->aauth_db->where('sekolah', _sekolah()->id);
		...
		if ($custom_where) {
			$this->aauth_db->where($custom_where);
		}

		$this->aauth_db->where('alumni', $alumni);
	...
	public function get_user_first_group($user_id = false){
		foreach($this->get_user_groups($user_id) as $row){
			return $row->name;
		}
		return false;
	}
	 ...
	 * @return bool
	 */
	public function is_ngadimin( $user_id = false ) {

		return $this->is_member(1, $user_id);
	}

	public function is_admin( $user_id = false ) {

		return $this->is_member(4, $user_id);
	}
