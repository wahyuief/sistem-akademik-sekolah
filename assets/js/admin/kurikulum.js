...
$('#btn-add-kurikulum').on('click', function(event) {
    $('#addKurikulumModalTitle').attr('action', base_url + 'admin/kurikulum/do_insert')
    $('#addKurikulumModal').find("input[type=text], input[type=number], input[type=email], input[type=password], textarea").val("")
    $('#addKurikulumModal #btn-add').show();
    $('#addKurikulumModal #btn-edit').hide();
	$('#addKurikulumModal').modal('show');
...
$(document).on("click", ".linkEdit", function (e, i) {
	...
	$('#addKurikulumModalTitle').attr('action', url)
	$('#tahun').val(tahun)
	$('#btn-add').removeAttr('disabled');
	$('#addKurikulumModal').modal('show');
})