...
$('#btn-add-pelajaran').on('click', function(event) {
    $('#addPelajaranModalTitle').attr('action', base_url + 'admin/pelajaran/do_insert')
    $('#addPelajaranModal').find("input[type=text], input[type=number], input[type=email], input[type=password], textarea").val("")
    $('#addPelajaranModal #btn-add').show();
    $('#addPelajaranModal #btn-edit').hide();
	$('#addPelajaranModal').modal('show');
...
$(document).on("click", ".linkEdit", function (e, i) {
	...
	$('#addPelajaranModalTitle').attr('action', url)
	$('#kode').val(kode)
	$('#nama').val(nama)
	$('#btn-add').removeAttr('disabled');
	$('#addPelajaranModal').modal('show');
